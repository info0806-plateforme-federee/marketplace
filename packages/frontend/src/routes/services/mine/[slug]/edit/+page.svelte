	...
	<form method="POST" use:enhance class="space-y-6">
		...
		<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
			<Input
				name="image"
				label="Docker Image"
				placeholder="python:3.14-slim, alpine:latest"
				value={service.image ?? ''}
			/>
			<Input
				name="command"
				label="Command"
				placeholder="Command to execute in the container"
		...
		<Textarea
			name="code"
			label="Code"
			placeholder="Inline code to execute (alternative to image)"
			value={service.code ?? ''}
			rows={6}
		/>
		...
		<Textarea
			...
			value={Object.keys(service.default_env).length > 0 ? JSON.stringify(service.default_env, null, 2) : ''}
			rows={4}
		/>
		...
		<div class="grid grid-cols-1 gap-4 md:grid-cols-3">
			<Input
				...
				type="number"
				placeholder="0"
				value={service.min_cpu != null ? String(service.min_cpu) : ''}
			/>
			<Input
				name="min_mem_mb"
				label="Min Memory MB"
				type="number"
				placeholder="0"
				value={service.min_mem_mb != null ? String(service.min_mem_mb) : ''}
			/>
			<Input
				name="min_gpu"
				label="Min GPU"
				type="number"
				placeholder="0"
				value={service.min_gpu != null ? String(service.min_gpu) : ''}
			/>
		</div>
		...
		<Input
			...
			type="number"
			placeholder="0"
			value={service.retry_count > 0 ? String(service.retry_count) : ''}
		/>
